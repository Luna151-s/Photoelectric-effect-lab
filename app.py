...
st.write("Enter your experimental data to calculate slope, Planck's constant, percentage error, and display your lab graph.")
...
method = st.sidebar.radio(
    "Choose Slope Method:",
    ("Endpoint Method (Max/Min Wavelengths)", "Linear Regression (Best-Fit Line)")
)
...
if st.button("🚀 Calculate & Generate Graph"):
    ...
    if len(df_clean) < 2:
        ...
        if method == "Endpoint Method (Max/Min Wavelengths)":
            # Uses lowest & highest wavelengths (Matches your manual 1.4% error logic)
            ...
            # Linear Best-Fit Line (Least Squares)
        ...
        ax.plot(freq_line, voltage_line, color='#FF5733', linestyle='--', linewidth=2, label=f'Slope Line ({method.split()[0]})')
